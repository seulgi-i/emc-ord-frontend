...
const CommonFields = ({ control }: FormComponentProps) => (
  <section className="p-4 border rounded-lg space-y-4">
    ...
    <InputField<OrderFormValues> control={control} id="orderMobilePhone" label="휴대폰 번호" type="tel" inputProps={{ maxLength: 11 }} />
  </section>
)
...
const DynamicFields = ({ orderType, control }: FormComponentProps) => {
  ...
  return (
    <section className="p-4 border rounded-lg space-y-4">
      ...
      {orderType !== 'E' && (
        <InputField<OrderFormValues> control={control} id="addr" label="주소" />
      )}
      {orderType === 'O' && (
...
const OrderForm = ({ orderType }: OrderFormProps) => {
  ...
  const { handleSubmit, watch, setValue, formState: { errors }, control, register } = methods
  ...
  if (isLoading) return <div className="text-center p-8">주문 정보를 불러오는 중...</div>
